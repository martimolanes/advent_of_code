...
fn main() {
    ...
    input
        .filter(|game| game_is_valid(game.as_ref().unwrap().clone()))
        .for_each(|game| {
...
fn game_is_valid(game: String) -> bool {
    ...
    game.split(";").for_each(|grab| {
        grab.split(",").for_each(|cubes_of_one_type| {
            ...
            match color {
                "red" => {
                    if num.parse::<u8>().unwrap() > max_color.red {
                        is_valid = false;
                    }
                }
                "green" => {
                    if num.parse::<u8>().unwrap() > max_color.green {
                        is_valid = false;
                    }
                }
                "blue" => {
                    if num.parse::<u8>().unwrap() > max_color.blue {
                        is_valid = false;
                    }
                }
